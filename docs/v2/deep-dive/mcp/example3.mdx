---
title: Example 3 - Loan Application Extended
sidebar_position: 8
---
...
export const loanExtendedDetails = {
  ...
  p: {
    ...
    session: ["state=PROCESS_APPLICATION", "context.agent.lifecycle active"],
  ...
  t1: {
    ...
    session: ["context.agent.observations[0]=credit rating"],
    tables: ["ce_audit (W)", "ce_verbose (optional R)"]
  },
  t2: {
    title: "Fraud check",
    ...
    session: ["context.agent.observations[1]=fraud result"],
    tables: ["ce_audit (W)"]
  },
  t3: {
    title: "Debt summary",
    ...
    session: ["context.agent.observations[2]=debt profile"],
    tables: ["ce_audit (W)"]
  },
  t4: {
    ...
    session: ["context.agent.observations[3]=submit result"],
    tables: ["ce_audit (W)", "ce_verbose (optional R)"]
  },
  post: {
    title: "Final MCP transition",
    stage: "POST_AGENT_MCP",
    summary: "When context.agent.finalAnswer exists, rules move PROCESS_APPLICATION -> COMPLETED.",
    session: ["state=COMPLETED", "context.agent.finalAnswer set"],
    tables: ["ce_rule (POST_AGENT_MCP)", "ce_audit (W)"]
  },
  done: {
    title: "Completed response",
    stage: "ASSISTANT_OUTPUT",
    summary: "The completed response uses context.agent.finalAnswer as the primary answer and validates against MCP observations.",
...
<DbTable
  ...
  rows={[
    ...
    ["COMPLETED", "Final answer", "Derived response uses context.agent.finalAnswer as the primary summary."]
  ]}
/>
...
<ChatContainer>
  ...
  <TraceChatBubble
    ...
    tables={["ce_rule (PRE_AGENT_MCP)", "ce_agent_planner (R)", "ce_agent_tool (R)", "ce_audit (W)", "ce_verbose (optional R)"]}
  ...
  <TraceChatBubble
    ...
    info={["POST_AGENT_MCP closes the flow only after context.agent.finalAnswer exists.", "COMPLETED response derives from final MCP context."]}
  />
</ChatContainer>
...
<DbTable
  ...
  rows={[
    ...
    ["Planner + tools", "ce_agent_planner, ce_agent_tool", "ce_audit", "Run the credit/fraud/debt/submit chain only after confirmation."],
    ["Post-MCP close", "ce_rule", "ce_audit", "POST_AGENT_MCP moves PROCESS_APPLICATION -> COMPLETED."],
    ["Final response", "ce_response, ce_prompt_template", "ce_conversation, ce_audit", "Return the final derived summary from context.agent.finalAnswer."]
  ]}
/>
...
If context.agent.finalAnswer exists, return it directly.
...
Use context.agent.finalAnswer as primary final answer.
Use context.agent.observations only to validate details.',
  0.00,
  'FINAL',
  '{"allows":["reset"],"expects":[]}',
  true,
...
(
  ...
  'DERIVED',
  NULL,
  'Use context.agent.finalAnswer as primary answer. Include applicationId when present.',
  NULL,
  10,
  true,
...
(
  ...
  'PROCESS_APPLICATION',
  'JSON_PATH',
  '$[?(@.context.agent.lifecycle.finished == true && @.context.agent.lifecycle.error == false && @.context.agent.finalAnswer != null && @.context.agent.finalAnswer != '''')]',
  'SET_STATE',
  'COMPLETED',
  10,
  true,
...
- `POST_AGENT_MCP` completion rule matches only when `context.agent.lifecycle.finished == true` and `context.agent.lifecycle.error == false`
...
- `AgentToolStep` then runs again in the same state and re-calls the API
