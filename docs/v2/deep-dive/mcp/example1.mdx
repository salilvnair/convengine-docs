...
export const liveExampleNodes = [
  ...
  { id: "m1", data: { label: "AgentToolStep: plan #1" } },
  { id: "t1", data: { label: "Tool call: mock.order.status" } },
  { id: "m2", data: { label: "AgentToolStep: plan #2" } },
  { id: "t2", data: { label: "Tool call: mock.order.async.trace" } },
  { id: "m3", data: { label: "AgentToolStep: ANSWER" } },
...
export const liveExampleDetails = {
  ...
  t2: {
    ...
    stage: "MCP_TOOL_RESULT",
    summary: "Second observation appended to context.agent.observations.",
    session: ["mcp.observations[1] written"],
    tables: ["ce_audit (W)"]
  },
  m3: {
    title: "AgentToolStep ANSWER",
    stage: "MCP_FINAL_ANSWER",
    summary: "Planner returns final answer and it is stored in context.agent.finalAnswer.",
  ...
  r2: {
    ...
    summary: "If context.agent.finalAnswer exists, rule moves ORDER_DIAGNOSTICS from ANALYZE to COMPLETED.",
...
3. seeding `ce_agent_tool` and planner prompts
...
That seed includes the live MCP tools and ORDER_DIAGNOSTICS response/rule wiring (ANALYZE -> COMPLETED via POST_AGENT_MCP when context.agent.finalAnswer exists):
...
<ChatContainer>
  ...
  <TraceChatBubble
    ...
    tables={["ce_agent_tool (R)", "ce_agent_planner (R)", "ce_audit (W)"]}
    info={["MCP loop runs until planner returns ANSWER.", "Observations are written in context.agent.observations."]}
  />
  <TraceChatBubble
    role="assistant"
    ...
    info={["POST_AGENT_MCP moves ANALYZE -> COMPLETED when context.agent.finalAnswer exists.", "ResponseResolutionStep uses context + derivation_hint."]}
  />
</ChatContainer>
...
<DbTable
  ...
  rows={[
    ["Turn 1 / Planner loop", "Planner selects status then async-trace tool.", "ce_agent_tool(R), ce_agent_planner(R), ce_audit(W)", "context.agent.observations[]"],
    ["Turn 1 / Planner answer", "Planner writes final MCP answer.", "ce_audit(W)", "context.agent.finalAnswer"],
    ["Turn 1 / Post-MCP rule", "POST_AGENT_MCP checks context.agent.finalAnswer and moves state to COMPLETED.", "ce_rule(R), ce_audit(W)", "state=COMPLETED"],
...
1. `AgentToolStep` clears stale `context.agent.*` at start of turn.
2. Planner (`AgentPlanner`) receives `user_input`, `context`, `mcp_tools`, and current `mcp_observations`.
3. Each `CALL_TOOL` result is appended into `context.agent.observations[]` as `{toolCode, json}`.
4. When planner returns `ANSWER`, step writes:
   - `context.agent.finalAnswer`
   - input param `mcp_final_answer`
5. `RulesStep` runs `POST_AGENT_MCP` phase; when context.agent.finalAnswer exists, rule transitions ANALYZE -> COMPLETED.
...
Even if your template does not explicitly reference `mcp_final_answer`, it still receives full context JSON. If `context.agent.observations` and `context.agent.finalAnswer` exist, response derivation can use them.
</Highlight>
...
<DbTable
  ...
  rows={[
    ...
    ["MCP loop #1", "Need order status first.", "ce_agent_tool(R), ce_agent_planner(R), ce_audit(W)", "CALL_TOOL mock.order.status"],
    ["Tool exec #1", "HTTP mapped status payload available.", "ce_audit(W)", "context.agent.observations[0]"],
    ["MCP loop #2", "Need callback trace to confirm async issue.", "ce_agent_tool(R), ce_agent_planner(R), ce_audit(W)", "CALL_TOOL mock.order.async.trace"],
    ["Tool exec #2", "Trace confirms callback missing/pending.", "ce_audit(W)", "context.agent.observations[1]"],
    ["MCP loop #3", "Enough evidence; produce conclusion.", "ce_agent_planner(R), ce_audit(W)", "context.agent.finalAnswer + mcp_final_answer"],
    ["Post-MCP rule", "When context.agent.finalAnswer exists, move ANALYZE to COMPLETED.", "ce_rule(R), ce_audit(W)", "state=COMPLETED"],
...
<DbTable
  ...
  rows={[
    ["ToolOrchestrationStep", "Executes exactly the requested tool without planner loop.", "ce_agent_tool(R), ce_audit(W)", "inputParams.tool_request"],
...
- planner path uses `context.agent.observations` and `context.agent.finalAnswer`
...
-- Planner path (AgentToolStep output)
UPDATE ce_prompt_template
SET user_prompt = 'Context JSON:\n{{context}}\n\nRead context.agent.observations and context.agent.finalAnswer. Produce a concise diagnostic summary.'
WHERE intent_code = 'ORDER_DIAGNOSTICS' AND state_code = 'COMPLETED' AND response_type = 'TEXT';
```

ORDER_DIAGNOSTICS is now included in demo seed with POST_AGENT_MCP completion and DERIVED final response from context.agent.finalAnswer.
