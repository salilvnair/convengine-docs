...
Use the current framework version from the repo: `convengine:2.0.20`
...
```kotlin
implementation("com.github.salilvnair:convengine:2.0.20")
```
...
(8, 'AgentPlanner', 'SYSTEM_PROMPT', 'You are an MCP planning agent inside ConvEngine...', true),
...
If utilizing the Semantic Query module, provision the semantic model DB tables (`ce_semantic_*`) using your semantic SQL bootstrap scripts (for example, `semantic_layer_yml_ddl.sql` + `semantic_layer_yml_dml.sql`). This abstracts specific physical model knowledge from generic engine functionality. See [Configuration](./configuration) for specifics.
