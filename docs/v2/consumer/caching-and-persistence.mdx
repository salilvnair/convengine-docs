...
1. <MethodRef>@CeEnableCaching</MethodRef>
2. <MethodRef>@CeEnableAsyncConversation</MethodRef>

<Highlight type="tip" title="How the abstraction works">
The actual datastore powering `@CeEnableCaching` is **entirely controlled by the Consumer application**, not the ConvEngine framework. ConvEngine relies on the generic Spring \`CacheManager\` interface to save and load \`CeConversation\` instances via the \`ce_conversation_cache\` routing key.
</Highlight>

## Default Configuration (HashMap)

By default, if you attach `@CeEnableCaching` but do not import any external caching dependencies into your `pom.xml`, Spring Boot falls back to a **`ConcurrentMapCacheManager`**.

This operates seamlessly out of the box. It holds conversation states inside a standard Java `HashMap` residing in your server's active JVM memory. 
