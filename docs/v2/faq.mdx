...
## Do `ce_agent_tool.intent_code` and `state_code` still support null wildcard behavior?
...
The same explicit-scope model also applies to `ce_agent_planner`.

## What is `ce_agent_planner` for?

`ce_agent_planner` is the scoped planner prompt source for MCP.
...
## What is the difference between `ToolOrchestrationStep` and `AgentToolStep`?
...
- writes `tool_result`, `tool_status`, and `context.agent.toolExecution.*`
...
- stores planner observations in `context.agent.observations`
- stores planner answer in `context.agent.finalAnswer`
