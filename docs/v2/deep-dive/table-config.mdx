...
- [Database Knowledge Graph (DBKG) metadata tables](#database-knowledge-graph-dbkg-metadata-tables)
...
<DbTable
  ...
  rows={[
    ["intent_code", "GREETING, LOAN_APPLICATION, ORDER_DIAGNOSTICS, DBKG_DIAGNOSTICS", "Primary intent identifier used everywhere else in scoped config."],
...
`ce_response` is still the final response authority even when MCP or Database Knowledge Graph (DBKG) computes internal answers.
...
<DbTable
  ...
  rows={[
    ...
    ["ce_mcp_sql_guardrail", "Read-only SQL safety extensions for MCP DB paths.", "McpSqlGuardrail, McpDbExecutor, DbkgQueryTemplateStepExecutor.", "rule_type, match_value, enabled, description"],
...
<DbTable
  ...
  rows={[
    ["ce_intent", "Which business intents exist.", "ORDER_DIAGNOSTICS, LOAN_APPLICATION, DBKG_DIAGNOSTICS, GREETING", "Every later scoped decision depends on a valid intent."],
...
<DbTable
  title="Database Knowledge Graph (DBKG) ce_mcp_* knowledge graph tables"
  columns={["Table", "Purpose", "Used by", "What it makes possible"]}
  rows={[
    ["ce_mcp_case_type", "Top-level investigation families.", "DbkgCaseResolver.", "Turns support language into a known case model."],
    ["ce_mcp_case_signal", "Weighted matching signals for cases.", "DbkgCaseResolver.", "Lets case resolution stay DB-driven."],
    ["ce_mcp_playbook", "Executable diagnostic playbooks.", "DbkgPlaybookResolver.", "Separates case family from exact diagnostic path."],
    ["ce_mcp_playbook_signal", "Weighted matching signals for playbooks.", "DbkgPlaybookResolver.", "Lets one case choose the right playbook."],
    ["ce_mcp_domain_entity", "Business nouns with synonyms, metadata_json, llm_hint.", "DbkgKnowledgeLookupService.", "Lets Database Knowledge Graph (DBKG) reason in business language, not just table names."],
    ["ce_mcp_domain_relation", "Edges between business entities.", "DbkgKnowledgeLookupService.", "Adds graph semantics across entities."],
    ["ce_mcp_system_node", "System/application registry.", "DbkgKnowledgeLookupService.", "Lets Database Knowledge Graph (DBKG) reason about system ownership and boundaries."],
    ["ce_mcp_api_flow", "Declarative API/call-chain metadata.", "DbkgKnowledgeLookupService.", "Gives Database Knowledge Graph (DBKG) system flow and downstream context."],
    ["ce_mcp_system_relation", "Edges between systems.", "DbkgKnowledgeLookupService.", "Models UI -> service -> downstream chains."],
    ["ce_mcp_db_object", "Optional table/view overlay metadata.", "DbkgKnowledgeLookupService.", "Adds semantic hints on top of live schema introspection."],
    ["ce_mcp_db_column", "Optional column overlay metadata.", "DbkgKnowledgeLookupService.", "Adds semantic names, synonyms, and hints without manually recreating schema."],
    ["ce_mcp_db_join_path", "Preferred join overrides.", "DbkgKnowledgeLookupService.", "Lets business-preferred joins override pure FK joins."],
    ["ce_mcp_status_dictionary", "Status code to label mapping.", "DbkgKnowledgeLookupService, Database Knowledge Graph (DBKG) param resolution.", "Turns codes like 200/400 into business meaning."],
    ["ce_mcp_id_lineage", "Cross-system identifier lineage.", "DbkgKnowledgeLookupService.", "Explains how request/order/disconnect ids propagate."],
    ["ce_mcp_executor_template", "Generic step executor registry.", "DbkgPlaybookEngine.", "Lets steps point to behavior by code instead of hardcoding."],
    ["ce_mcp_query_template", "Approved SQL templates.", "DbkgPlaybookEngine.", "Keeps DB execution deterministic and auditable."],
    ["ce_mcp_query_param_rule", "How query params are filled.", "DbkgPlaybookEngine.", "Lets params come from args, status dictionaries, context, or prior outputs."],
    ["ce_mcp_playbook_step", "Playbook DAG nodes.", "DbkgPlaybookResolver, DbkgPlaybookValidator, DbkgPlaybookEngine.", "Defines each executable step in the investigation graph."],
    ["ce_mcp_playbook_transition", "Playbook DAG edges.", "DbkgPlaybookValidator, DbkgPlaybookEngine.", "Controls execution order and branching."],
    ["ce_mcp_outcome_rule", "Final diagnosis rules.", "DbkgOutcomeResolver.", "Turns runtime results into stable explanations."]
  ]}
/>
...
### `convengine.mcp.db.knowledge-graph.*`

<DbTable
  title="Database Knowledge Graph (DBKG) config"
  columns={["Setting / value", "Possible values", "What it changes"]}
  rows={[
    ["enabled", "true / false", "Turns the Database Knowledge Graph (DBKG) tool chain on or off."],
    ["sqlGuardrailTable", "ce_mcp_sql_guardrail", "Points the shared SQL guard at the table that governs read-only function and keyword rules for all MCP DB execution paths."],
    ["schemaIntrospectionEnabled", "true / false", "Lets Database Knowledge Graph (DBKG) read live schema from JDBC metadata."],
    ["includedSchemas / excludedSchemas", "list of schema names", "Controls which database schemas Database Knowledge Graph (DBKG) scans."],
    ["schemaObjectLimit", "integer", "Caps how many physical objects schema discovery returns."],
    ["caseResolveToolCode / knowledgeLookupToolCode / investigatePlanToolCode / playbookValidateToolCode / investigateExecuteToolCode", "string tool codes", "Lets you remap Database Knowledge Graph (DBKG) tool codes without rewriting handlers."],
    ["table name overrides", "string table names", "Lets you remap Database Knowledge Graph (DBKG) metadata table names if your schema naming differs."],
    ["maxResults / scanLimit", "integer", "Limits metadata ranking and prevents low-value result flooding."]
  ]}
/>
...
<DbTable
  ...
  rows={[
    ...
    ["Database Knowledge Graph (DBKG) investigation routing", "ce_mcp_case_* + ce_mcp_playbook*", "That is where Database Knowledge Graph (DBKG) chooses the case and the playbook path."],
    ["Database Knowledge Graph (DBKG) SQL and diagnosis logic", "ce_mcp_query_template + ce_mcp_query_param_rule + ce_mcp_outcome_rule", "That controls safe execution and the final explanation."],
