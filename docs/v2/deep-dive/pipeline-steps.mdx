...
<DbTable
  ...
  rows={[
    ...
    ["McpToolStep", "MCP planner/tool loop", "context_json.mcp.*", "ce_mcp_tool, ce_mcp_db_tool, ce_mcp_planner (fallback ce_config)"],
...
Rules execute by phase. Available native phases include `PRE_RESPONSE_RESOLUTION`, `POST_AGENT_INTENT`, `POST_AGENT_MCP`, and `POST_TOOL_EXECUTION`.
</Highlight>
