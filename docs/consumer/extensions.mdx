...
import { CodeBlockToggle, Highlight, FlowStep, DbTable, MethodRef, FileRef } from "@site/src/components/convengine";
...
## 5) Rule Action Playbook (`SET_TASK`, `SET_JSON`, `GET_CONTEXT`, `GET_SESSION`)
...
### 5.1 `SET_TASK` - execute consumer Java methods from rule
...
<CodeBlockToggle
  ...
  language="java"
  packagePath="com.acme.convengine.tasks"
...
### 5.2 `SET_JSON` - move JSONPath value into input params
...
### 5.3 `GET_CONTEXT` - snapshot context into input params
...
### 5.4 `GET_SESSION` - snapshot session facts into input params
...
Resolvers: <FileRef>engine/rule/type/provider/SetTaskActionResolver.java</FileRef>, <FileRef>engine/rule/type/provider/SetJsonActionResolver.java</FileRef>, <FileRef>engine/rule/type/provider/GetContextActionResolver.java</FileRef>, <FileRef>engine/rule/type/provider/GetSessionActionResolver.java</FileRef>.  
Task invocation: <FileRef>engine/rule/task/CeRuleTaskExecutor.java</FileRef>.  
Execution loop: <FileRef>engine/steps/RulesStep.java</FileRef>.
</Highlight>

### 5.5 Custom action resolver (`RuleActionResolver`)
...
<CodeBlockToggle
  ...
  language="java"
  packagePath="com.acme.convengine.rules"
...
No manual factory config is needed. <MethodRef>RuleActionResolverFactory</MethodRef> auto-discovers all Spring beans implementing <MethodRef>RuleActionResolver</MethodRef> and maps by <MethodRef>action().toUpperCase()</MethodRef>.
</Highlight>
