...
- Semantic Query runtime (full pipeline):
  ...
  - Semantic model loader: `SemanticModelLoader` + `SemanticModelRegistry` load `semantic-layer.yaml`.
...
- Semantic model administration endpoints:
  - `POST /api/v1/db/semantic-query/generate-model` – LLM-driven draft generation of `semantic-layer.yaml` from live DB schema.
...
- MCP planner enhancements:
  ...
  - DBKG capsule: planner receives compacted `dbkg_capsule` observation context from `DbkgSupportService`.
...
- SQL audit helper:
  - new `McpSqlAuditHelper` utility consolidates SQL execution audit payload generation across `McpDbExecutor`, `DbkgQueryTemplateStepExecutor`, and `PostgresQueryToolHandler`.
- Schema knowledge enhancements:
  - `DbSemanticCatalogToolHandler` and `DbSemanticCatalogService` for semantic-catalog mode.
  - `DbkgCaseResolveToolHandler`, `DbkgInvestigateExecuteToolHandler`, `DbkgInvestigatePlanToolHandler`, `DbkgKnowledgeLookupToolHandler`, `DbkgPlaybookValidateToolHandler` new DBKG tool handlers.
  - `DbkgQueryTemplateStepExecutor` enhanced SQL execution auditing.
  - `DefaultPostgresQueryInterceptor` + `PostgresQueryInterceptor` for query interception.
  - `DefaultSemanticCatalogVectorSearchInterceptor` for catalog vector search.
  - schema knowledge valid values and vector support in `DbkgSchemaCatalogService`.
...
- DDL and seed expansions:
  ...
  - DBKG dedicated DDL/seeds: `mcp/dbkg/ddl_postgres.sql`, `mcp/dbkg/ddl_sqlite.sql`, `mcp/dbkg/seed.sql`.
  - schema-aware SQL seed: `dbkg/schema_aware_sql_seed.sql`.
  - semantic model resource: `mcp/semantic-layer.yaml`.
...
- Verbose and audit coverage expansion:
  ...
  - new MCP SQL observability stages: `MCP_DB_SQL_EXECUTION` and `DBKG_QUERY_SQL_EXECUTION`
