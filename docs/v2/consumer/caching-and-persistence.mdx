...
import { DbTable, CodeBlockToggle, Highlight, MethodRef,
  FlowStep
...
<FlowStep step="1" title="Ehcache 3 (JSR-107)">
...
<FlowStep step="2" title="Caffeine Cache">
...
<FlowStep step="4" title="Static Configuration Preloading">


Beyond dynamic conversational sessions, ConvEngine also permanently caches all non-transactional database rules (`ce_intent`, `ce_rule`, `ce_agent_tool`, `ce_agent_planner`, `ce_config`, etc.) into a generic `ce_static_configs` memory tree entirely upon JVM live initiation. This ensures that the engine never triggers synchronous relational fetches during the middle of a customer's turn.
...
<FlowStep step="5" title="Cache Analyzer API (/api/v1/cache/analyze)">
...
<CodeBlockToggle title="Sample response" language="json">
