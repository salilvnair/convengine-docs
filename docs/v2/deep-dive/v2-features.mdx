...
- `McpToolStep` hard-blocks `dbkg.investigate.execute` if a successful validation observation is missing
...
- MCP became planner-scoped with `ce_mcp_planner`
- MCP scope validation became strict for `ce_mcp_tool` and `ce_mcp_planner`
- guardrail-blocked next-tool behavior became deterministic
- richer MCP lifecycle metadata became available in `context.mcp.*`
