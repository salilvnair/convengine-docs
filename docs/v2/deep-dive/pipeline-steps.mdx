...
- **`EDIT`**: User wants to modify previous input ("wait change that to 5").
- **`RESET`**: User wants to start over.

**Resolution flow (Config to LLM):**
1. By default, reads `convengine.flow.dialogue-act.resolute` from `application.yml` (e.g., `REGEX_THEN_LLM`).
2. Checks fast regexes first in `classifyByRegex(userText)`:
   - `(?i)^(\\s)*(yes|yep|yeah|ok|okay|sure|go ahead|do that|please do|confirm|approved?)(\\s)*$` maps to `AFFIRM` (0.95d confidence).
   - `(?i)^(\\s)*(no|nope|nah|cancel|stop|don't|do not)(\\s)*$` maps to `NEGATE` (0.95d).
   - `(?i)^(\\s)*(edit|revise|change|modify|update)(\\s)*$` maps to `EDIT` (0.95d).
3. If regex confidence is below `convengine.flow.dialogue-act.llm-threshold`, it falls back to `classifyByLlm(session, userText)`.
4. The LLM is prompted to probabilistically resolve the `DialogueAct` using a strict JSON schema generator.
5. The result is stored as `DIALOGUE_ACT`, `DIALOGUE_ACT_CONFIDENCE`, and `DIALOGUE_ACT_SOURCE` in the engine session's input params for downstream rules to evaluate.
...
3. If no custom matrix decision applies, it checks boolean flags from the config.
   ...
   - `isFillPendingSlotOnNonNewRequest()`: If true, `!NEW_REQUEST` + `hasPendingSlot` -> `FILL_PENDING_SLOT`
