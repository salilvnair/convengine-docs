---
title: Example 2 - Loan Application
sidebar_position: 7
---
...
export const loanExampleDetails = {
  ...
  r2: {
    ...
    summary: "After planner ANSWER, POST_AGENT_MCP rule checks context.agent.finalAnswer and moves state from ELIGIBILITY_GATE to COMPLETED.",
  ...
  m1: {
    ...
    tables: ["ce_agent_tool (R)", "ce_agent_planner (R)", "ce_audit (W)"]
  },
  t1: {
    title: "loan.credit.rating.check",
    stage: "MCP_TOOL_RESULT",
    summary: "Mapped rating response appended to context.agent.observations.",
  ...
  m2: {
    ...
    tables: ["ce_agent_planner (R)", "ce_audit (W)"]
  },
  t2: {
  ...
  m5: {
    ...
    summary: "Planner synthesizes final decision and stores context.agent.finalAnswer.",
...
- `ce_response`
- `ce_rule`
- `ce_agent_tool` (with `intent_code` + `state_code`)
- `ce_agent_planner` scoped prompt rows (default + LOAN_APPLICATION/ELIGIBILITY_GATE)
...
INSERT INTO ce_rule (intent_code, state_code, rule_type, match_pattern, action, action_value, phase, priority, enabled, description)
VALUES
('LOAN_APPLICATION', 'ELIGIBILITY_GATE', 'JSON_PATH', '$[?(@.context.agent.finalAnswer != null && @.context.agent.finalAnswer != '''')]', 'SET_STATE', 'COMPLETED', 'POST_AGENT_MCP', 30, true,
 'Move loan flow to COMPLETED only when context.agent.finalAnswer exists.');
...
('LOAN_APPLICATION', 'COMPLETED', 'TEXT', 'DERIVED',
 'Use context.agent.finalAnswer as primary summary. Validate with context.agent.observations (rating/fraud/debt/submit). Do not invent values.',
 20, true, 'Loan decision derived from MCP outputs.');

-- 4) Prompt template should explicitly read context.agent.*
...
('LOAN_APPLICATION', 'COMPLETED', 'TEXT',
 ...
 'Context JSON:\n{{context}}\n\nRead context.agent.observations and context.agent.finalAnswer. Return a concise final loan decision.',
 'FINAL',
...
<ChatContainer>
  ...
  <TraceChatBubble
    ...
    tables={["ce_agent_tool (R)", "ce_agent_planner (R)", "ce_audit (W)"]}
    info={["Planner decides each next tool from previous observation.", "Observations are persisted in context.agent.observations."]}
  />
  <TraceChatBubble
    role="assistant"
...
<DbTable
  ...
  rows={[
    ...
    ["Post-MCP transition", "POST_AGENT_MCP rule moves state to COMPLETED only when context.agent.finalAnswer is present.", "ce_rule(R), ce_audit(W)", "state=COMPLETED"],
    ["MCP tool chain", "Rating, fraud, debt, submit executed conditionally.", "ce_agent_tool(R), ce_agent_planner(R), ce_audit(W)", "context.agent.observations[]"],
    ["MCP final answer", "Planner writes final decision text.", "ce_audit(W)", "context.agent.finalAnswer"],
...
1. `AgentToolStep` starts and clears stale `context.agent.finalAnswer` and `context.agent.observations`.
2. Every successful tool call appends one entry into `context.agent.observations`:
   - `toolCode`
   - `json` (stringified mapped payload)
3. When planner returns `ANSWER`, `AgentToolStep` writes:
   - `context.agent.finalAnswer`
   - input param `mcp_final_answer`
4. In the confirmation-first pattern, `POST_SCHEMA_EXTRACTION` can move the flow to `CONFIRMATION`, `PRE_AGENT_MCP` can move `CONFIRMATION -> PROCESS_APPLICATION`, and `POST_AGENT_MCP` can set `COMPLETED` when `context.agent.finalAnswer` is present.
...
In this setup, the response LLM is grounded by two things together: `derivation_hint` policy from `ce_response` and MCP evidence from `context.agent.*`. That is why the final answer reflects rating/fraud/debt/applicationId consistently.
</Highlight>

The provided Example 2 seed now makes this explicit in SQL as well:

- `ce_prompt_template.user_prompt` explicitly instructs reading `context.agent.observations` and `context.agent.finalAnswer`
- `ce_response.derivation_hint` explicitly references `context.agent.*`
...
<DbTable
  ...
  rows={[
    ...
    ["MCP #1", "Need credit rating first.", "ce_agent_tool(R), ce_agent_planner(R), ce_audit(W)", "CALL_TOOL rating"],
    ["Tool #1", "Rating available and > 750.", "ce_audit(W)", "obs[0]=creditRating"],
    ["MCP #2", "Proceed to fraud check.", "ce_agent_planner(R), ce_audit(W)", "CALL_TOOL fraud"],
    ["Tool #2", "Fraud clear.", "ce_audit(W)", "obs[1]=flagged:false"],
    ["MCP #3", "Need affordability metrics.", "ce_agent_planner(R), ce_audit(W)", "CALL_TOOL debt summary"],
    ["Tool #3", "DTI acceptable and credit sufficient.", "ce_audit(W)", "obs[2]=dti/availableCredit"],
    ["MCP #4", "Submit application now.", "ce_agent_planner(R), ce_audit(W)", "CALL_TOOL submit"],
    ["Tool #4", "Submission succeeded.", "ce_audit(W)", "obs[3]=applicationId/status"],
    ["MCP ANSWER", "Enough evidence for final decision.", "ce_audit(W)", "context.agent.finalAnswer"],
...
<DbTable
  ...
  columns={["Loop/Step", "LLM deduction", "Tables touched", "Produced state/data"]}
  rows={[
    ["MCP #1", "Call rating check.", "ce_agent_tool(R), ce_agent_planner(R), ce_audit(W)", "CALL_TOOL rating"],
    ["Tool #1", "creditRating <= 750.", "ce_audit(W)", "obs[0]=low rating"],
    ["MCP ANSWER", "Stop chain; reject without fraud/debt/submit.", "ce_agent_planner(R), ce_audit(W)", "context.agent.finalAnswer (rejection)"],
...
<DbTable
  ...
  columns={["Loop/Step", "LLM deduction", "Tables touched", "Produced state/data"]}
  rows={[
    ["MCP #1 + Tool #1", "Rating is high enough to continue.", "ce_agent_tool(R), ce_agent_planner(R), ce_audit(W)", "obs[0]=rating"],
    ["MCP #2 + Tool #2", "Fraud flagged=true.", "ce_agent_planner(R), ce_audit(W)", "obs[1]=fraud flagged"],
    ["MCP ANSWER", "Stop chain immediately; reject.", "ce_agent_planner(R), ce_audit(W)", "context.agent.finalAnswer (fraud rejection)"],
...
<DbTable
  ...
  columns={["Loop/Step", "LLM deduction", "Tables touched", "Produced state/data"]}
  rows={[
    ["MCP #1 + Tool #1", "Rating allows continuation.", "ce_agent_tool(R), ce_agent_planner(R), ce_audit(W)", "obs[0]=rating"],
    ["MCP #2 + Tool #2", "Fraud clear.", "ce_agent_planner(R), ce_audit(W)", "obs[1]=fraud false"],
    ["MCP #3 + Tool #3", "DTI too high or available credit too low.", "ce_agent_planner(R), ce_audit(W)", "obs[2]=poor affordability"],
    ["MCP ANSWER", "Reject or mark manual review; skip submit.", "ce_agent_planner(R), ce_audit(W)", "context.agent.finalAnswer"],
...
`ce_agent_tool` rows in example2 are scoped as:
...
- No tool call in planner path:
  - check `AgentPlanner` prompt rows in `ce_agent_planner`
...
- [MCP Basics](/docs/v2/deep-dive/mcp/basics)
- [MCP Advanced Guide](/docs/v2/deep-dive/mcp/advanced)
- [MCP Example 1 (Mockey + Demo)](/docs/v2/deep-dive/mcp/example1)
