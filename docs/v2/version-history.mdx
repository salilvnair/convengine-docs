...
- Advanced MCP execution model:
  ...
  - HTTP handler models:
    - `HttpApiApiProcessorToolHandler` ([api-processor](https://github.com/salilvnair/api-processor) / RestWebServiceFacade path)
...
- MCP docs expansion:
  ...
  - new dedicated [MCP Deep Dive](/docs/v2/consumer/mcp/deep-dive) with chat-by-chat flow, table impact, and step graph.
