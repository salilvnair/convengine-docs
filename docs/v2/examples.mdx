...
<div className="ce-step-badge-list" role="list" aria-label="Canonical runtime step set">
  ...
  <div className="ce-step-badge-item"><span className="ce-step-badge-index">19</span><span className="ce-step-badge-label">AgentToolStep</span></div>
...
<Conversation title="DB Orchestration Flow">
  ...
  <Assistant intent="DATABASE_QUERY" state="IDLE" tables={["ce_agent_tool"]} info={["Tool orchestration starts.", "System preparing DB call."]}>Checking the inventory database...</Assistant>
...
INSERT INTO ce_prompt_template (intent_code, state_code, response_type, system_prompt, user_prompt, temperature, enabled)
VALUES ('DATABASE_QUERY', 'COMPLETED', 'DERIVED', 'You are an inventory assistant.', 'Answer from these database results: {{context.agent.finalAnswer}}', 0.0, true);

INSERT INTO ce_response (intent_code, state_code, output_format, response_type, derivation_hint, priority, enabled)
VALUES ('DATABASE_QUERY', 'COMPLETED', 'TEXT', 'DERIVED', 'Render the final answer from context.agent.finalAnswer.', 10, true);
```
