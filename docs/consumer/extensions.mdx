...
- <MethodRef>PIPELINE_RULES</MethodRef>: normal <MethodRef>RulesStep</MethodRef> pass.
- <MethodRef>AGENT_POST_INTENT</MethodRef>: post-intent pass inside <MethodRef>AgentIntentResolver</MethodRef>.
...
VALUES
  ('PIPELINE_RULES', 'REQUEST_TRACKER', 'REGEX', '(?i).*track.*request.*', 'SET_TASK', 'requestTrackerTask:loadStatus,attachEta', 10, true,
...
{`INSERT INTO ce_rule
  (phase, intent_code, rule_type, match_pattern, action, action_value, priority, enabled, description)
VALUES
  ('PIPELINE_RULES', 'LOG_ANALYSIS', 'JSON_PATH', '$.schemaJson.errorCode != null', 'SET_JSON', 'error_code:$.schemaJson.errorCode', 20, true,
...
{`INSERT INTO ce_rule
  (phase, intent_code, rule_type, match_pattern, action, action_value, priority, enabled, description)
VALUES
  ('PIPELINE_RULES', 'REQUEST_TRACKER', 'REGEX', '(?i).*status.*', 'GET_CONTEXT', 'ctx_snapshot', 30, true,
...
{`INSERT INTO ce_rule
  (phase, intent_code, rule_type, match_pattern, action, action_value, priority, enabled, description)
VALUES
  ('PIPELINE_RULES', 'DISCONNECT_ELECTRICITY', 'JSON_PATH', '$.schemaComplete == false', 'GET_SESSION', 'session_snapshot', 40, true,
