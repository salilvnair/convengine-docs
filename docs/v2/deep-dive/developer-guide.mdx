...
<DbTable
  ...
  rows={[
    ...
    ["interaction-policy.*", "Map acts to system outcomes like EXECUTE_PENDING_ACTION.", "matrix: PENDING_ACTION:AFFIRM -> EXECUTE"],
    ["action-lifecycle.*", "Evict old confirmations out of memory.", "ttl-turns: 3"],
    ["disambiguation.*", "Ask the user to clarify between colliding priorities.", "max-options: 5"],
    ["guardrail.*", "Prevent prompt-injection and sanitize text.", "approval-gate-fail-closed: true"],
    ["state-graph.*", "Enforce strict state transition bounds.", "soft-block-on-violation: false"],
  ]}
/>
...
- **Phase Targeting**: Do not depend on one rule only; cleanly separate constraints into `PRE_EVAL`, `EVAL`, and `POST_EVAL` rule phases.
