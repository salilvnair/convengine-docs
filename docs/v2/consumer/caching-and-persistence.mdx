...
If you annotate your application class with \`@CeEnableCaching\` but fail to include \`spring-boot-starter-cache\` in your \`pom.xml\`, the Spring context will fatally crash on startup because it cannot find an underlying \`CacheManager\` bean to bind to.
</Highlight>
