...
- `ce_mcp_planner`
- `ce_verbose`
- Database Knowledge Graph (DBKG) (`dbkg.*`)
...
If you are onboarding with Database Knowledge Graph (DBKG), also apply and seed the Database Knowledge Graph (DBKG) package:

- `sql/mcp/dbkg/ddl_postgres.sql` or `sql/mcp/dbkg/ddl_sqlite.sql`
- `sql/mcp/dbkg/seed.sql`

If you want the live Zapper-style test schema used in the Database Knowledge Graph (DBKG) docs/examples:

- `sql/zapper/ddl_postgres.sql` or `sql/zapper/ddl_sqlite.sql`
- `sql/zapper/seed.sql`
...
## 9. If you adopt Database Knowledge Graph (DBKG), start even narrower

Database Knowledge Graph (DBKG) is powerful, but it should still start as one controlled vertical slice.

Good first Database Knowledge Graph (DBKG) rollout:

- one investigative intent
- one or two case types
- one playbook
- one or two query templates
- one explicit validation path
- one response path for success and one for blocked/invalid playbook

Bad first Database Knowledge Graph (DBKG) rollout:

- many case types on day one
- dozens of playbooks before you have trace discipline
- no `ce_mcp_outcome_rule` coverage
- direct ad hoc SQL outside `ce_mcp_query_template`
- project-specific investigation logic hardcoded into Java handlers

## 10. Use `ce_verbose` early
...
## 11. Validate with a real pre-production checklist
...
Add the `2.0.10` dependency, enable ConvEngine, and provide a production-grade `LlmClient`.
</FlowStep>
...
<FlowStep step="6" title="Add Database Knowledge Graph (DBKG) only after you can explain one full trace end-to-end">
Enable Database Knowledge Graph (DBKG) after your team can read the audit trail, understand the playbook graph, and explain why a validation or execute step was chosen.
</FlowStep>

## Database Knowledge Graph (DBKG)-ready starter config

<CodeBlockToggle title="Starter config with Database Knowledge Graph (DBKG) disabled by default" language="yaml">
{`convengine:
  mcp:
    guardrail:
      enabled: false
