...
<DbTable
  ...
  rows={[
    ...
    ["POST", "/feedback", "Submit user feedback on a conversation turn"],
...
<DbTable
  title="DB schema and semantic model administration endpoints"
  columns={["Method", "Path", "Purpose"]}
  rows={[
    ["GET", "/inspect-schema", "Introspect live DB schema (tables, columns, FKs). Optional `prefix` and `schema` query params."],
    ["POST", "/agent", "Generate seed SQL from DB schema using LLM agent."],
    ["POST", "/semantic/embeddings/rebuild", "Rebuild pgvector embeddings from semantic model."],
    ["POST", "/semantic-query/generate-model", "LLM-driven draft generation of `semantic-layer.yaml` from live DB schema."],
    ["POST", "/semantic-query/validate-model", "Validate semantic model YAML against physical schema."],
    ["POST", "/semantic-query/save-model", "Persist semantic model YAML to configured path."],
    ["POST", "/semantic-query/reload-model", "Hot-reload semantic model into runtime registry."],
    ["GET", "/semantic-query/current-model-yaml", "Return current active semantic model YAML."]
  ]}
/>

<Highlight type="info" title="Requires semantic mode">
Semantic model administration endpoints require `convengine.mcp.db.semantic.enabled=true` in `application.yml`.
</Highlight>
