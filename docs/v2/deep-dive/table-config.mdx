...
<DbTable
  ...
  rows={[
    ...
    ["config_type", "AgentPlanner, Flow, Audit, etc.", "Groups related config keys."],
...
`ce_agent_tool` is the scoped MCP tool registry. In the current v2 line, scope is explicit and mandatory.

<DbTable
  title="ce_agent_tool columns"
  columns={["Column", "Typical values", "What it does"]}
  rows={[
...
`ce_agent_db_tool` is the SQL-template fallback configuration when the DB tool path is template-driven.

<DbTable
  title="ce_agent_db_tool columns"
  columns={["Column", "Typical values", "What it does"]}
  rows={[
    ["tool_id", "FK to ce_agent_tool.tool_id", "Binds the DB-specific config to a tool row."],
...
### `ce_agent_sql_guardrail`

`ce_agent_sql_guardrail` is the shared SQL safety table for MCP DB execution. It keeps the runtime read-only while still letting you explicitly allow or block SQL functions and extra SQL keywords.

<DbTable
  title="ce_agent_sql_guardrail columns"
  columns={["Column", "Typical values", "What it does"]}
  rows={[
...
<DbTable
  title="ce_agent_sql_guardrail example values"
...
```sql
INSERT INTO ce_agent_sql_guardrail (
  rule_type,
  match_value,
  enabled,
  description
...
`ce_agent_planner` makes planner prompts data-driven and scope-aware.

<DbTable
  title="ce_agent_planner columns"
  columns={["Column", "Typical values", "What it does"]}
  rows={[
...
<DbTable
  ...
  rows={[
    ...
    ["ce_agent_tool", "MCP tool registry with strict intent/state scope.", "AgentToolStep, ToolOrchestrationStep, AgentToolRegistry.", "tool_code, tool_group, intent_code, state_code, enabled"],
    ["ce_agent_db_tool", "SQL-template fallback configuration for DB tools.", "McpDbToolExecutor / DB tool fallback paths.", "tool_id, dialect, sql_template, param_schema, safe_mode, max_rows"],
    ["ce_agent_sql_guardrail", "Read-only SQL safety extensions for MCP DB paths.", "McpSqlGuardrail, McpDbExecutor, SemanticQueryTemplateStepExecutor.", "rule_type, match_value, enabled, description"],
    ["ce_agent_planner", "Planner prompts scoped by intent/state.", "AgentPlanner.", "planner_id, intent_code, state_code, system_prompt, user_prompt, enabled"],
...
<DbTable
  ...
  rows={[
    ...
    ["ce_agent_tool", "Which tools are visible for a given intent/state.", "tool_group values: DB, HTTP_API, WORKFLOW_ACTION, DOCUMENT_RETRIEVAL, CALCULATOR_TRANSFORM, NOTIFICATION, FILES", "Prevents cross-intent tool leakage and drives both direct and planner MCP flows."],
    ["ce_agent_db_tool", "How DB tools can fall back to deterministic SQL templates.", "dialect values such as postgres/sqlite/oracle depending on consumer setup", "Used when DB execution is template-driven rather than handler-driven."],
    ["ce_agent_sql_guardrail", "How read-only SQL is further constrained or extended.", "rule_type values: ALLOW_FUNCTION, BLOCK_FUNCTION, BLOCK_KEYWORD", "Lets you stay read-only while still governing which SQL functions and extra keywords are permitted."],
    ["ce_agent_planner", "How planner prompts change by domain and state.", "resolution order is exact scope -> intent+ANY -> ANY+ANY -> legacy ce_config fallback", "This makes MCP planning data-driven instead of globally hardcoded."]
  ]}
/>
...
<DbTable
  ...
  rows={[
    ...
    ["Direct tool availability", "ce_agent_tool + convengine.flow.tool-orchestration.*", "That controls tool visibility and direct execution."],
    ["Planner behavior", "ce_agent_planner + convengine.mcp.guardrail.*", "That controls tool order and whether transitions are allowed."],
