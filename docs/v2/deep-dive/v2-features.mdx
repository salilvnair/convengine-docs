...
### `ce_agent_tool` scope rules (correct current behavior)

The old statement about optional `intent_code` / `state_code` with `NULL` wildcard is no longer true.

Current behavior:

- `ce_agent_tool.intent_code` is mandatory
- `ce_agent_tool.state_code` is mandatory
...
### `ce_agent_planner` (newer v2 addition)

MCP prompt selection is now data-driven via `ce_agent_planner`.
...
3. `ANY + ANY`
4. legacy fallback from `ce_config` `AgentPlanner` keys when planner rows are unavailable

Like `ce_agent_tool`, `ce_agent_planner.intent_code` and `state_code` are mandatory and validated at startup.
...
- resolves scoped tools from `AgentToolRegistry`
- runs a bounded planner/tool loop (`MAX_LOOPS = 5`)
- stores observations in `context.agent.observations`
- stores final planner answer in `context.agent.finalAnswer`
...
## 6. Database Knowledge Graph (DBKG) Turns MCP Into a DB-Driven Investigation Engine

This is the biggest newer addition missing from older v2 summaries.

Database Knowledge Graph (DBKG) (`dbkg.*`) extends MCP from "tool calling" into a configurable investigation runtime for production support and data debugging.

Current Database Knowledge Graph (DBKG) flow:
...
What makes Database Knowledge Graph (DBKG) different from legacy DB lookup:

- it resolves a business case first, not raw SQL first
- it uses live schema introspection plus optional semantic overlays
- it chooses a playbook from DB metadata
- it validates the playbook graph before execution
- it executes only approved query templates and derived steps
- it resolves final outcomes from DB-stored diagnosis rules

Current Database Knowledge Graph (DBKG) control-plane tables include:

- case resolution:
  - `ce_mcp_case_type`
  - `ce_mcp_case_signal`
- playbook selection and graph:
  - `ce_mcp_playbook`
...
Current Database Knowledge Graph (DBKG) runtime guard model:

- planner prompt sequencing is DB-driven
- `DbkgPlaybookValidator` validates the graph before execute
- `AgentToolStep` hard-blocks `dbkg.investigate.execute` if a successful validation observation is missing
- the block reason is explicit (`DBKG_VALIDATION_REQUIRED_BEFORE_EXECUTION`)

If you need the full Database Knowledge Graph (DBKG) model, use the dedicated deep dives:

- [MCP](/docs/v2/deep-dive/mcp)
- [MCP Semantic Query](/docs/v2/deep-dive/mcp/semantic-query)
- [Database Knowledge Graph (DBKG) Table Configuration](/docs/v2/deep-dive/table-config)
- [MCP Example 4 - Zapper Disconnect](/docs/v2/deep-dive/mcp/example4)

## 7. Rules Are Broader Than the Original v2 Rule Model
...
## 8. Schema, Prompt, and Response Handling Are Richer in Current v2
...
## 9. Verbose Runtime and Step Telemetry Are First-Class Features
...
## 10. Memory, Continuity, and Reset Handling Expanded
...
## 11. Cache, Startup Validation, and Operational Safety Are Core v2 Features
...
## 12. Consumer Integration and Extension Points Are Broader
...
## 13. Release-Line Additions That Matter (2.0.6 to 2.0.9)

The current repo includes several important changes from the later v2 line that are easy to miss if you only read older high-level docs.

### 2.0.6 line

- cache reliability and diagnostics became first-class operational features
- static cache analysis and refresh tooling were added for faster environment debugging
- cache/proxy behavior was tightened so static `ce_*` config lookups stay stable after warmup

### 2.0.7 line

- MCP became planner-scoped with `ce_agent_planner`
- MCP scope validation became strict for `ce_agent_tool` and `ce_agent_planner`
- guardrail-blocked next-tool behavior became deterministic
- richer MCP lifecycle metadata became available in `context.agent.*`
- advanced MCP HTTP handler models were added

### 2.0.8 line

- `ce_verbose` was added as a true control-plane table
- verbose events started flowing through SSE and STOMP, not just audit rows
- step telemetry was added to runtime session state
- MCP schema completeness checks became stricter and deterministic

### 2.0.9 line

- `CorrectionStep` became a real routing layer instead of a documentation gap
- prompt-template interaction semantics (`interaction_mode`, `interaction_contract`) became part of actual runtime flow
- prompt rendering consolidated around the shared Thymeleaf-backed renderer
- Database Knowledge Graph (DBKG) was added as the DB-driven MCP investigation model for business-to-database diagnostics

## 14. What Changed From the Old Page
...
- it missed `ce_agent_planner`
- it missed Database Knowledge Graph (DBKG) and the DB-backed knowledge graph / playbook model
