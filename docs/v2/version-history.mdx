...
- **Framework Caching & Async Pipelines**: 
  - Separated Spring's raw `@EnableCaching` and `@EnableAsync` from the default ConvEngine auto-configuration.
  - Introduced explicitly managed `@CeEnableCaching` and `@CeEnableAsyncConversation` consumer integration decorators.
