...
<DbTable
  title="MCP and runtime extension points"
  columns={["Extension", "Where it runs", "Typical consumer use"]}
  rows={[
    ["`SemanticCatalogVectorSearchInterceptor`", "`db.semantic.catalog` ranking path", "Use pgvector or custom ANN retrieval instead of framework cosine fallback."],
    ["`PostgresQueryInterceptor`", "`postgres.query` pre-guardrail execution path", "Normalize/repair SQL, enforce schema prefixes, tenant constraints, org SQL policy."],
    ["`DbToolHandler`", "MCP DB tool execution path", "Register new DB-backed MCP tool_code handlers in Java."],
    ["`HttpApiRequestingToolHandler` / `HttpApiProcessorToolHandler`", "MCP HTTP tool path", "Add custom request/response mapping logic for HTTP tools."],
    ["`MessageResolver`", "verbose message resolution", "Custom verbose token mapping and templated payload behavior."],
  ]}
/>

## `SemanticCatalogVectorSearchInterceptor` - custom vector ranking {#61-semanticcatalogvectorsearchinterceptor---custom-vector-ranking}

Real scenario: use PGVector cosine distance directly from your DB instead of Java fallback ranking.

<CodeBlockToggle title="SemanticCatalogVectorSearchInterceptor example" language="java" packagePath="com.zapper.convengine.mcp">
{`@Component
...
public class PgVectorSemanticSearchInterceptor implements SemanticCatalogVectorSearchInterceptor {
    ...
    public List<Map<String, Object>> rank(
        ...
        String embedding = llmClient.generateEmbedding(question); // consumer-controlled impl
        String table = "query".equalsIgnoreCase(sourceType)
                ? cfg.getQueryCatalogTable()
                : cfg.getSchemaCatalogTable();
        int k = Math.max(1, cfg.getVectorSearch().getMaxResults());

        String sql = "SELECT t.*, 1 - (t.vector <=> CAST(:embedding AS vector)) AS _vector_score " +
                "FROM " + table + " t " +
                "ORDER BY t.vector <=> CAST(:embedding AS vector) " +
                "LIMIT :k";

        return jdbcTemplate.queryForList(sql, Map.of("embedding", embedding, "k", k));
    }
}`}
</CodeBlockToggle>

## `PostgresQueryInterceptor` - normalize/fix SQL before execution {#62-postgresqueryinterceptor---normalizefix-sql-before-execution}
...
<CodeBlockToggle title="PostgresQueryInterceptor example" language="java" packagePath="com.zapper.convengine.mcp">
{`@Component
...
<Highlight type="tip" title="Order and fallback">
...
<DbTable
  title="Other consumer extension interfaces available"
  columns={["Interface", "Purpose"]}
  rows={[
