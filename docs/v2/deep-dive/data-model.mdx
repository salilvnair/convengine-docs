...
- `ce_agent_planner` became the scoped planner prompt source
- MCP scope rules for `ce_agent_tool` and `ce_agent_planner` became strict instead of nullable-wildcard
...
- `ce_agent_sql_guardrail` remains the shared SQL read-only extension layer for MCP DB tools
...
`ce_agent_tool.tool_group` normalizes to:
...
`ce_agent_tool` uses mandatory scope columns in the current v2 line:
...
`ce_agent_planner` supports scoped planner prompts with the same strict scope model:
