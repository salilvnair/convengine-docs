...
- `ce_agent_tool`, `ce_agent_planner`, `ce_pending_action`, and `ce_verbose` are startup-validated for scope/integrity
...
- `ce_agent_planner` makes MCP prompt selection deterministic by intent/state scope instead of relying only on legacy config
...
- `context.agent.lifecycle.*` and `context.agent.toolExecution.*`
...
<DbTable
  ...
  rows={[
    ...
    [
      ...
      "`ce_agent_tool` design + consumer-side handler implementation"
    ],
    [
