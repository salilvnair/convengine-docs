...
4. `RulesStep` runs `POST_AGENT_MCP` phase and matches rule only when `context.mcp.finalAnswer` is present, then sets state to `COMPLETED`.
...
<DbTable
  ...
  rows={[
    ...
    ["State rule", "loan flow evaluates in ELIGIBILITY_GATE (pre-MCP), then POST_AGENT_MCP moves to COMPLETED only when finalAnswer exists.", "ce_rule(R), ce_audit(W)", "state=ELIGIBILITY_GATE"],
