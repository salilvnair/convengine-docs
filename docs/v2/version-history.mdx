...
2.0.20 is the CE Builder Studio, Agent Builder Studio foundation, and semantic layer expansion release. Compared to 2.0.12, this adds the CE Builder visual canvas, Agent Builder Studio backend, expanded semantic DB table layer, and semantic model DB-overlay loader.
</div>

- Added **CE Builder Studio** (`ce-builder/`) — drag-and-drop canvas for authoring `ce_*` configuration table rows visually and deploying them directly to the running backend via bulk upsert + cache refresh.
- Added **Agent Builder Studio** backend controllers (`AgentBuilderStudioController`, `BuilderStudioController`) exposing workspace sync/load, one-shot agent calls, full-graph run, and LLM provider management under `/api/v1/builder-studio/*`.
- Added `ce-builder-studio` standalone Fastify/TypeScript server mirroring the builder REST surface for lightweight local development.
- Expanded semantic layer DB tables: added `ce_semantic_model`, `ce_semantic_setting`, `ce_semantic_source_table`, `ce_semantic_source_column`, `ce_semantic_lexicon`, `ce_semantic_rule_allowed_table`, `ce_semantic_rule_deny_operation`, `ce_semantic_rule_config` loaded by `SemanticModelDynamicOverlayService`.
- Added `ce_semantic_join_path` to `SemanticTableNames`, used by `SemanticLlmQueryService` SQL compilation.
- Added `SemanticEmbeddingService` (replaces `SemanticEmbeddingDbService`) with embedding rebuild, catalog refresh, query failure embedding, and user-query knowledge indexing.
- Added Builder Studio DB entities: `CeBsWorkspace`, `CeBsWorkflow`, `CeBsAgent`, `CeBsAgentPool`, `CeBsAgentSkill`, `CeBsSkill`, `CeBsTeam`, `CeBsLlmConfig`.

## 2.0.12

<div className="ce-schema ce-intro-box">
2.0.12 is the semantic-docs simplification and cleanup release. Compared to 2.0.11, this removes stale semantic-v1/DBKG documentation paths, rewrites semantic deep-dive pages around the current runtime flow, and aligns v2 docs with the active semantic configuration and API traces.
</div>

- Removed stale documentation content tied to legacy semantic-v1/DBKG paths and old catalog pages.
- Reworked MCP semantic docs to reflect the active runtime chain: `db.semantic.interpret -> db.semantic.query -> postgres.query`.
- Rewrote semantic deep-dive content to show turn-by-turn trace, planner handoff, and sequential code path without tab-fragmented flow.
- Updated API documentation pages (including audit/trace coverage) to match current MCP/semantic stage events.
- Refreshed v2 navigation/index artifacts after cleanup (`sidebars.v2.js`, docs index generation outputs).

## 2.0.11

<div className="ce-schema ce-intro-box">
2.0.11 is the transition-query and semantic-query hardening release. Compared to 2.0.10, this adds robust status-transition query handling, stronger SQL regeneration guards, and failure-memory feedback improvements for Agent-2 SQL generation.
</div>

- Added transition-intent support for status movement scenarios (for example, "went from 810 to 120") using semantic filter fields like `fromStatus` and `toStatus`.
- Added/normalized semantic interpreter contract fields for support handling:
  - `operationSupported`
  - `unsupportedMessage`
  - consistent clarification payload fields.
- Strengthened semantic SQL generation with:
  - retry with error feedback
  - SQL candidate validation against mapped base-table columns
  - safer timestamp/date parameter handling in generated SQL.
- Improved failure-memory loop through `ce_semantic_query_failures` with correction tracking and embedding-assisted example retrieval.
- Added operational controls via `ce_config` keys for query retries and failure-example selection (`SQL_RETRY_MAX_ATTEMPTS`, `FAILURE_EXAMPLE_TOP_K`, `FAILURE_EXAMPLE_CANDIDATE_LIMIT`).

## 2.0.10

<div className="ce-schema ce-intro-box">
2.0.10 is the DB-driven semantic foundation release. Compared to 2.0.9, this introduces metadata-driven semantic interpretation/query generation with explicit guardrails, semantic control tables, and prompt/schema contracts persisted in DB.
</div>

- Introduced metadata-driven semantic interpretation and query runtime centered on:
  - `db.semantic.interpret`
  - `db.semantic.query`
  - `postgres.query`
- Added DB-driven semantic control model (`ce_semantic_*`) for entities, fields, query classes, mappings, synonyms, ambiguity options, and embeddings.
- Added prompt and output contract control via DB:
  - `ce_prompt_template`
  - `ce_output_schema`
  - unified clarification response shape (selected option key, clarification answer text, etc.).
- Added strict semantic guardrail payload injection (`allowed_entity_keys`, `allowed_fields_by_entity`, semantic metadata blocks) so LLM output is constrained by DB metadata.
- Added semantic query failure memory persistence (`ce_semantic_query_failures`) as feedback foundation for later correction/retry improvements.
- Added semantic model loading path from runtime resource and aligned docs/examples to the active semantic model.
...
- Verbose publishing across core pipeline and resolvers:
  ...
  - tool + MCP lifecycle (`ToolOrchestrationStep`, `AgentToolStep`)
...
- MCP schema completeness hardening:
  ...
  - `AgentToolStep` now skips MCP execution when schema-required fields are missing and writes lifecycle metadata deterministically.
...
- MCP planner scoping table:
  - `ce_agent_planner` with intent/state scoped prompt selection.
...
- Guardrail behavior clarity:
  - when planner proposes blocked next tool, fallback answer is written to `context.agent.finalAnswer`.
...
- Tool execution observability upgrade:
  - direct tool mode writes rich metadata into `context.agent.toolExecution.*` including `toolCode`, `toolGroup`, status/outcome, scope mismatch flags, and tool row metadata.
- Scope consistency hardening in static config:
  - `ce_agent_tool` / `ce_agent_planner` scope values are validated at load/startup (`intent_code`, `state_code`) against allowed forms (`ANY`, `UNKNOWN`, configured values).
...
- **Static Configurations Cache Loader**: All static framework properties (`ce_intent`, `ce_rule`, `ce_agent_tool`, etc.) are pre-loaded entirely into memory on JVM live via `StaticTableCachePreloader`, dropping complex query I/O across `RulesStep` and `InteractionPolicyStep` to sub-millisecond evaluate times.
...
- **`CeAgentTool` Scope Isolation**: The `intent_code` and `state_code` columns were physically integrated into the `ce_agent_tool` table architecture to formally mandate extreme compartmentalization of tool access limits. Tools are strictly bound to domains and are no longer assumed "global by default."
- **`AgentToolRepository` Refactoring**: Updated JPQL definitions in the repository (`findEnabledByIntentAndState`, `findByToolCodeEnabledAndIntentAndState`) leveraging the Spring Data `@Param` annotation to guarantee tools are flawlessly filtered. The internal `AgentPlanner` is logically isolated from discovering tools sitting externally to the current active conversational domain intent scope.
- **`AgentToolRegistry` Improvements**: A hardened centralized registry mechanism uniquely responsible for standardizing `tool_group` domains and resolving perfectly legal tools for a given conversation cycle loop, mathematically preventing hallucinations where an LLM invokes an unbound tool.
