...
<DbTable
  ...
  rows={[
    ...
    ["ToolOrchestrationStep", "Run tool_group based orchestration", "tool_request/tool_result fields", "ce_tool, ce_agent_tool, ce_audit"],
    ["AgentToolStep", "MCP planner/tool loop", "context_json.mcp.*, mcp tool metadata", "ce_agent_tool, ce_agent_db_tool, ce_agent_planner (fallback ce_config)"],
...
<Highlight type="info" title="Prompt-template interaction metadata">
...
4. Most crucially, if any of these policy decisions trigger, it sets `skipIntentResolution = true`. This stops Step 13 (`IntentResolutionStep`) from overriding the sticky intent. The payload is attached with `POLICY_DECISION`.
...
<div className="ce-step-badge-list" role="list" style={{ marginBottom: '1rem' }}>
  <div className="ce-step-badge-item">
    ...
    <span className="ce-step-badge-label" style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>CorrectionStep</span>
  </div>
</div>

**Responsibility:** Route confirmation turns and apply in-place correction patches  
**Session Mutations:** `routing_decision, skip_schema_extraction, correction_applied`  
**Config/Table Dependencies:** `ce_output_schema, session context, ce_audit`  

<details open>
<summary>Detailed Execution Logic</summary>

This step handles confirmation-aware correction before the engine re-enters normal intent and schema processing. It is the bridge between "the user replied to a prior question" and "the engine should patch the existing structured request instead of treating the reply as a brand-new request."

What it does:
- Inspects the active prompt-template interaction contract to understand whether the turn is confirming, editing, retrying, or continuing collection.
- Looks at current session context, pending schema facts, and correction markers already present in the conversation.
- Decides whether to patch prior extracted fields in place, keep them as-is, or let later steps continue with full extraction.
- Sets routing hints so downstream steps know whether schema extraction should be skipped or whether corrected values are already in context.

Why it matters:
- Without this step, a user saying "No, use Irving not Dallas" would often be reclassified as a new request instead of a correction to the existing structured input.
- It keeps correction behavior data-driven through prompt-template interaction semantics instead of hardcoded state-name parsing.

<CodeBlockToggle title="CorrectionStep.execute()" language="java" filePath="src/main/java/com/github/salilvnair/convengine/engine/steps/CorrectionStep.java" defaultOpen={true}>

```java
public StepResult execute(EngineSession session) {
        // inspect interaction contract + pending structured context
        CorrectionDecision decision = correctionResolver.resolve(session);

        session.putInputParam("routing_decision", decision.routingDecision());
        session.putInputParam("skip_schema_extraction", decision.skipSchemaExtraction());
        session.putInputParam("correction_applied", decision.correctionApplied());

        if (decision.updatedContext() != null) {
            session.mergeContext(decision.updatedContext());
        }

        audit.audit(ConvEngineAuditStage.CORRECTION_APPLIED, session.getConversationId(), mapOf(
                "routingDecision", decision.routingDecision(),
                "skipSchemaExtraction", decision.skipSchemaExtraction(),
                "correctionApplied", decision.correctionApplied()
        ));
...
**Config/Table Dependencies:** `ce_tool, ce_agent_tool, ce_audit`  
...
public StepResult execute(EngineSession session) {
        ...
        try {
            CeAgentTool tool = request.toolCode() == null || request.toolCode().isBlank()
                    ? null
...
<div className="ce-step-badge-list" role="list" style={{ marginBottom: '1rem' }}>
  <div className="ce-step-badge-item">
    ...
    <span className="ce-step-badge-label" style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>AgentToolStep</span>
  </div>
</div>

**Responsibility:** MCP planner/tool loop  
**Session Mutations:** `context_json.mcp.*`  
**Config/Table Dependencies:** `ce_agent_tool, ce_agent_db_tool, ce_agent_planner (fallback ce_config)`  

<details open>
<summary>Detailed Execution Logic</summary>

Specifically iterates over `ce_agent_tool` bindings. Instead of static grouped tools, this triggers an agent planner that interprets the input, selects an MCP tool, writes the payload, and executes it. This is the core of dynamic tool use in ConvEngine V2.

<CodeBlockToggle title="AgentToolStep.execute()" language="java" filePath="src/main/java/com/github/salilvnair/convengine/engine/steps/AgentToolStep.java" defaultOpen={true}>
...
public StepResult execute(EngineSession session) {
        ...
        List<AgentObservation> observations = readObservationsFromContext(session);
        boolean mcpTouched = false;

        for (int i = 0; i < MAX_LOOPS; i++) {

            AgentPlan plan = planner.plan(session, tools, observations);
            mcpTouched = true;
            ...
            try {
                ...
                observations.add(new AgentObservation(toolCode, rowsJson));
...
- `interaction_contract` is where consumers should declare capabilities such as `retry` on `PROCESSING` prompts or `affirm/edit` on `CONFIRM` prompts
...
<div className="ce-step-badge-list" role="list" style={{ marginBottom: '1rem' }}>
  <div className="ce-step-badge-item">
    <span className="ce-step-badge-index">27</span>
