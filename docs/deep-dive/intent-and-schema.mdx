...
<DbTable
  ...
  columns={["config_key", "Purpose", "Example"]}
  rows={[
    ["STICKY_INTENT", "Keep resolved intent/state stable across turns and skip unnecessary re-resolution", "true"],
  ]}
/>
...
- lock is released
- if `STICKY_INTENT=true`, resolver can still skip and retain existing intent/state
