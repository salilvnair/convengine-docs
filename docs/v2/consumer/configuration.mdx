...
## CE Verbose Runtime Messaging (v2.0.8)

`2.0.8` adds a new control-plane table, `ce_verbose`, to drive deterministic runtime progress/error messages from the Java framework pipeline.

This is not a UI-only feature. The framework resolves verbose messages in backend runtime and emits them through SSE/STOMP payloads.

### What `ce_verbose` controls

<DbTable
  title="ce_verbose matching inputs"
  columns={["Column", "Meaning", "Typical values"]}
  rows={[
    ["intent_code", "Intent scope filter", "Exact intent or `ANY`"],
    ["state_code", "State scope filter", "Exact state or `ANY`"],
    ["step_match", "Step matching mode", "`EXACT`, `REGEX`, `JSON_PATH`"],
    ["step_value", "Step target expression", "`RulesStep`, `McpToolStep`, `.*Step$`, or JSONPath expression"],
    ["determinant", "Runtime event determinant", "`STEP_ENTER`, `RULE_APPLIED`, `MCP_TOOL_CALL`, `AGENT_INTENT_ACCEPTED`, etc."],
    ["rule_id", "Optional strict rule match", "Specific `ce_rule.rule_id`"],
    ["tool_code", "Optional strict tool match", "Specific `ce_mcp_tool.tool_code`"],
    ["message / error_message", "Info/error text emitted to stream", "Human-readable progress text"],
    ["priority", "Tie-break order (lower is stronger)", "10, 20, 100"],
    ["enabled", "Row active flag", "true/false"]
  ]}
/>

### CE IDE SQL panel workflow (DDL + DML)

If your team uses CE IDE SQL panel tabs:

1. Open SQL panel `DDL` tab.
2. Run `src/main/resources/sql/verbose_ddl.sql` (or the equivalent block from your dialect `ddl*.sql`).
3. Open SQL panel `DML` tab.
4. Run `src/main/resources/sql/verbose_seed.sql` (or the corresponding `seed*.sql` rows for your dialect).
5. Refresh cache (`POST /api/v1/cache/refresh`) so `ce_verbose` rows are loaded immediately.

<CodeBlockToggle title="Postgres standalone DDL (ce_verbose)" language="sql">
...
<CodeBlockToggle title="Sample DML rows (ce_verbose)" language="sql">
...
- `EnginePipelineFactory` writes step enter/exit/error telemetry into `session.stepInfos`.
- `VerboseMessagePublisher` builds a resolve request (`intent`, `state`, `step`, `determinant`, `ruleId`, `toolCode`, metadata).
- `DbVerboseMessageResolver` picks the highest-priority eligible row from `ce_verbose`.
- Stream transport (`SSE` / `STOMP`) emits event envelope with `eventType="VERBOSE"` and `verbose` payload.

### Important compatibility notes

- Keep `step_match` limited to `EXACT|REGEX|JSON_PATH`; startup validation rejects invalid values.
- `ce_verbose` is part of static cache preload in `2.0.8`; missing table/invalid rows can fail startup.
- Keep verbose text operational and short; this payload can be streamed to end-user channels.
