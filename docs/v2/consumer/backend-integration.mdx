...
import { CodeBlockToggle, Highlight, FlowStep, FileRef, MethodRef, DbTable } from "@site/src/components/convengine";
...
## Consumer Semantic Endpoints

Semantic Query model-admin APIs are exposed from `DbSchemaController` under:

`/api/v1/db/semantic-query`

<DbTable
  title="Semantic model admin APIs"
  columns={["Method", "Path", "Purpose"]}
  rows={[
    ["POST", "/generate-model", "Generate semantic YAML draft from inspected schema + metadata hints."],
    ["POST", "/validate-model", "Validate semantic YAML and return errors/warnings with diagnostics."],
    ["POST", "/save-model", "Persist semantic YAML model version/name in config storage."],
    ["POST", "/reload-model", "Reload active semantic model into runtime (request YAML / saved / model-path)."]
  ]}
/>
