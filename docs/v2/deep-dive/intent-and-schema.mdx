...
When the LLM path is engaged (confidence below `llm-threshold`), it must return a strict JSON payload containing the act and confidence to be parsed by the engine.
