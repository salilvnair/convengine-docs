...
import { CodeBlockToggle, Highlight, MethodRef,
  FlowStep
...
- `ce_agent_tool` / `ce_agent_planner` only when testing MCP
...
<FlowStep step="1" title="Rule and state debugging">
...
<FlowStep step="2" title="Prompt and LLM debugging">
...
<FlowStep step="3" title="MCP debugging">
...
For SQL-backed MCP and Semantic Query queries, inspect the audit payload metadata: `sql`, `params`, `row_count`, and `rows`.
...
<FlowStep step="4" title="Direct tool debugging">
